...
| AI 看图问答 | ✅ 悬浮窗 | — | 灯箱里对图片直接提问（多模态模型）：多会话保存/重命名/清空/删除，时序排列，md 渲染 |
| 图片留言板 | ✅ 悬浮窗 | — | 灯箱里为图片留存多条相关信息：Markdown 渲染、收起/展开、`#标签` 标记与筛选 |
...
**AI 看图问答**：灯箱查看图片时点「🤖 AI 问答」（仅图片可用），弹出可拖拽的悬浮窗，直接对这张图片提问（如「图里写了什么」「白板上的要点整理成列表」）。由多模态模型识图回答，回答流式显示，完成后按 Markdown 渲染（表格/标题/代码块/引用等，暗色适配）。支持**多会话**：顶栏可切换会话（按最近更新时序排列）、新建（＋）、重命名（✎）、清空重置（🧹）、删除（🗑）；每个图片的会话持久化在 `attachments/.chat/<图片名>.json`（不入库），下次打开自动恢复，删除图片时同步清理。首次提问自动以问题前 20 字建会话。HEIC 自动转码、过大图片自动压缩后送入模型。需配置 API Key 且模型支持图片输入（如 deepseek-flash）。

**图片留言板**：灯箱查看图片时点「📝 留言板」（仅图片可用），弹出可拖拽的悬浮窗，为这张图片留存多条相关信息。每条留言支持 **Markdown 渲染**（表格/标题/加粗等，暗色适配），可用 `#标签` 标记（如「白板要点 #待办 #重点」），点标签可筛选留言，再点清除；每条可单独收起/展开（标题栏 ▾ 可全部收起/展开），状态持久化。数据本地保存在该会议 `attachments/.comments/<图片名>.json`（不入库），删除图片时自动同步清理。
